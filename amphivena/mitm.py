...
class MitM:
    def __init__(self, interface1, interface2=None):
        ...
        try:
            subprocess.run('ip address show dev ' + shlex.quote(self._interface1),
    ...
    def teardown(self):
        ...
        if subprocess.run('ip address show ' + shlex.quote(self.bridge_name),
            ...
            try:
                subprocess.run('ip link set ' + shlex.quote(self.bridge_name) + ' down',
                               capture_output=True, shell=True, check=True)
                subprocess.run('ip link delete ' + shlex.quote(self.bridge_name) + ' type bridge',
                               capture_output=True, shell=True, check=True)
    ...
    def kernel_br_module_up(self):
        ...
        try:
            ...
            if subprocess.run('test -d /proc/sys/net/bridge', capture_output=True, shell=True).returncode == 0:
                ...
                self.__kernel_br_ipv6 = subprocess.run('cat /proc/sys/net/bridge/bridge-nf-call-ip6tables',
                                                       capture_output=True, shell=True, check=True).stdout
    ...
    def kernel_br_module_down(self):
        ...
        try:
            if self.__kernel_bridge_previously_enabled:
                subprocess.run('echo ' + shlex.quote(self.__kernel_br_ipv4.decode("utf-8").rstrip()) + ' > /proc/sys/net/bridge/bridge-nf-call-iptables',
                               capture_output=True, shell=True, check=True)
                subprocess.run('echo ' + shlex.quote(self.__kernel_br_ipv6.decode("utf-8").rstrip()) + ' > /proc/sys/net/bridge/bridge-nf-call-ip6tables',
                               capture_output=True, shell=True, check=True)
                subprocess.run('echo ' + shlex.quote(self.__kernel_br_arp.decode("utf-8").rstrip()) + ' > /proc/sys/net/bridge/bridge-nf-call-arptables',
                               capture_output=True, shell=True, check=True)
            else:
    ...
    def network_tap(self):
        ...
        try:
            subprocess.run('ip link add ' + shlex.quote(self.bridge_name) + ' type bridge',
                           capture_output=True, shell=True, check=True)
            subprocess.run('ip link set ' + shlex.quote(self._interface1) + ' master ' + shlex.quote(self.bridge_name),
                           capture_output=True, shell=True, check=True)
            subprocess.run('ip link set ' + shlex.quote(self._interface2) + ' master ' + shlex.quote(self.bridge_name),
                           capture_output=True, shell=True, check=True)
            subprocess.run('ip link set ' + shlex.quote(self.bridge_name) + ' up',
                           capture_output=True, shell=True, check=True)
