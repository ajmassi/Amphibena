...
def test_ordered_edit_tcp_consecutive_packets(
    mock_accept, new_packet_processor, new_tls_client_hello, new_tls_server_hello
):
    """
    ...
    packet_processor = new_packet_processor.get(
        "packet_processing/ordered_edit_tcp_consecutive_packets.json"
    )
    ...
    assert IP(mock_accept.call_args[0][0].get_payload()).getlayer(TCP).sport == 12345
    ...
    packet_processor._process(pkt)
    assert IP(mock_accept.call_args[0][0].get_payload()).getlayer(TCP).dport == 12345


@mock.patch("amphivena.packet_processor.PacketProcessor._drop")
def test_ordered_drop_tcp(
    mock_drop, new_packet_processor, new_tls_client_hello, new_tls_server_hello
):
    """Drop TCP packet with source port 443."""
    packet_processor = new_packet_processor.get(
        "packet_processing/ordered_drop_tcp.json"
    )
...
@mock.patch("amphivena.packet_processor.PacketProcessor._drop")
@mock.patch("amphivena.packet_processor.PacketProcessor._finalize")
def test_ordered_drop_tcp_later_packet(
    mock_accept,
    mock_drop,
    new_packet_processor,
    new_tls_client_hello,
    new_tls_server_hello,
):
    """
...
@mock.patch("amphivena.packet_processor.PacketProcessor._finalize")
def test_ordered_edit_tcp_multiple_steps_same_packet(
    mock_accept, new_packet_processor, new_tls_client_hello
):
    """
    ...
    """
    packet_processor = new_packet_processor.get(
        "packet_processing/ordered_edit_tcp_multiple_steps_same_packet.json"
    )
    ...
    assert IP(mock_accept.call_args[0][0].get_payload()).getlayer(TCP).sport == 12345
    assert IP(mock_accept.call_args[0][0].get_payload()).getlayer(IP).ttl == 255
